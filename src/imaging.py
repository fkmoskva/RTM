...
def cross_correlate(forward_snaps, adjoint_snaps, decompress_fn=None):
    common_steps = sorted(set(forward_snaps) & set(adjoint_snaps))
    if not common_steps:
        return np.zeros_like(next(iter(forward_snaps.values())))
    image = np.zeros_like(forward_snaps[common_steps[0]])
    for step in common_steps:
        snap_fwd = decompress_fn(forward_snaps[step]) if decompress_fn else forward_snaps[step]
        image   += snap_fwd * adjoint_snaps[step]
    return image


'''Метрики качества: rel_error, RMSE, PSNR.'''
def metrics(image, reference):
    ...
    return dict(rel_error=rel_err, rmse=rmse, psnr=psnr)
